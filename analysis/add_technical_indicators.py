...
#Moving Average  
def MA(df, n):  
    MA = pd.Series(df['Close'].rolling(window=n, center=False).mean(), name = 'SMA_' + str(n))  
    df = df.join(MA)  
    return df
...
#Bollinger Bands  
def BBANDS(df, n):  
    MA = pd.Series(df['Close'].rolling(window=n, center=False).mean())  
    MSD = pd.Series(df['Close'].rolling(window=n, center=False).std())  
    b1 = 4 * MSD / MA  
